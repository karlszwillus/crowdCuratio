...
|--------------------------------------------------------------------------
|
| Block E / Welle E.3 — Admin-Edit-Pfad auf `PATCH /users/{user}`.
| Authorize: nur Admin. Felder: firstName / lastName / optional
| roles / optional adminUser / optional createProject.
*/
...
it('authorize: Admin darf', function () {
    ...
    $request = UpdateUserAsAdminRequest::create('/users/1', 'PATCH');
...
it('rules: firstName und lastName sind pflicht', function () {
    /** @var TestCase $this */
    $request = new UpdateUserAsAdminRequest;
...
it('rules: roles, adminUser, createProject sind optional', function () {
    /** @var TestCase $this */
    $request = new UpdateUserAsAdminRequest;

    expect($request->rules())->toHaveKey('roles');
    expect($request->rules()['roles'])->toContain('sometimes');
});
