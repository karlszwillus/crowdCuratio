...
Q4-Etappe 8 · E3e (2026-09-12): Rollen-Bearbeiten von Bootstrap-3
auf Tailwind umgezogen; zusammenhaengendes Formular statt zwei
`@section`-Bloecken.
--}}
...
@section('main')
    <div class="mx-auto max-w-2xl">
        <h2 class="mb-6 text-title font-semibold text-ink-900">{{ __('edit_role') }}</h2>
                    ...
                    @foreach ($permission as $value)
                        <label class="inline-flex items-start gap-2 rounded-md border border-line-200 bg-canvas-bg px-3 py-2 text-body text-ink-900 hover:bg-line-100">
                            <input type="checkbox"
                                   ...
                                   @checked(in_array($value->permission_id, old('permission', $rolePermissions ?? [])))
